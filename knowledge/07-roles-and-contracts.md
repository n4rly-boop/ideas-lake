...
| **B · Граф и хранилище**                        | B                 | схема хранения, Neo4j, переоценка. **Индекс тезисов — не здесь**: он у A (C2, решено 2026-07-28 вечером). **Рёбра Idea—Idea теперь пишет A** (решено D12, 2026-07-31): co-citation в фазе 2 и `derived_from` при синтезе, в самом пайплайне, не отдельным скриптом | зелёный                                 |
...
- **[изменено, D12, решение владельца A] Рёбра `(:Idea)-[:RELATED]->(:Idea)` теперь пишет A, в своём пайплайне.** Со-встречаемость больше не отдельным полем и не выводится из батча кем-то другим: A сама вызывает `graph_client.write_cocitation_edges(source_id)` сразу после того, как все идеи источника закоммичены (фаза 2, `ingest/run.py`), обе стороны, вес накапливается идемпотентно по источнику. `derived_from` (гипотеза → два родителя) A пишет в момент синтеза (`idea_merger.write_hypothesis`), а не постфактум разбором `locator` синтетического листа. Старый текст ниже — до D12, оставлен как история: «Со-встречаемость отдельным полем не передаётся: батч приходит одним вызовом на один источник, все затронутые идеи в нём видны, инициализацию рёбер B выводит из батча.»
- Пере-вывод текста идеи инициирует **A** по накоплению 3 новых листьев; флаг `dirty` и пересчёт `trust_score` — на **B**.
- **[решено 2026-07-28]** Формат `Idea` больше не блокирует A: поля согласованы с B, зафиксированы в `06` §3.3 и `10` §1. За **B** остаются таймстемпы и архив. **[изменено D10/D11/D14, 2026-07-31]** `dirty` пишет A при создании и пере-выводе (фаза 2, `lake/ingest/run.py`); `trust_score` считает LLM-судья A в конце фазы 2 (по идеям с `dirty=True`, `lake/ingest/trust.py`); рёбра `derived_from` и cocitation пишет A в пайплайне (D12 выше).
