...
> [!IMPORTANT]
> ### Отчёт по проекту → [`docs/Ideas-Lake-report.pdf`](docs/Ideas-Lake-report.pdf)
>
> Полное описание системы, постановка эксперимента, результаты и границы их валидности.
> 14 страниц, английский. Исходник — [`docs/report.md`](docs/report.md).

![Карта системы](docs/fig/system_map.png)
...
Озеро на 2026-08-02: **901 идея, 3 336 тезисов из 535 источников, 15 176 рёбер.**
...
### Запись: источник → тезисы → идеи

![Путь записи](docs/fig/write_path_strip.png)

Фаза 1 — скачивание, парсинг, обобщение; в граф не пишет ничего. Фаза 2 — арбитр решает,
к какой идее относится тезис, идея с листьями пишется одной транзакцией, дальше рёбра
со-встречаемости и судья доверия. Отказ арбитра → `pending_link`, а не «новая идея».

### Чтение: запрос → выдача

![Путь чтения](docs/fig/read_path_strip.png)

Переписывание запроса → гибридный поиск по тезисам (FTS5 + вектора, RRF) → подъём к идеям →
ранжирование → добор соседями по рёбрам, если идей меньше `k`.

## Контракт наружу

### `POST /retrieve` — контракт C3
...
### `POST /research` — агент озера

![Агент озера](docs/fig/agent_lake_strip.png)
