...
`/ui` — пятый открытый путь, наряду с `/docs` и `/openapi.json`, и по той же причине: браузер
...
только на `GET`, соседние пути и другие методы по-прежнему `401`, страницы нет в OpenAPI, в
ней нет ни одного id из озера, и её JavaScript разбирается `node --check` — синтаксическая
ошибка в одном файле означала бы пустую страницу под честным `200`.
