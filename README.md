# Ideas Lake — блок A: Ingestion + Retrieve

AIRI Summer 2026, проект 28 «Озеро идей». Защита — 3 августа 2026.

Долговременная память между прогонами эволюции GigaEvo: статья → тезис → обезличенная
идея на запись, запрос → выдача с провенансом на чтение.

Владелец: Александр Малый. Соседние блоки — граф и Neo4j (B), эволюция (C), стоимость (D).
...
| `lake/graph_client.py` | единственное место, знающее формат хранилища B |
| `lake/prompts/` | промпты текстовыми файлами, не строками в коде |
| `lake/selfcheck.py` | assert-проверки инвариантов, один запуск |

Спека реализации, контракты между блоками и разборы прототипов-доноров — в локальной
базе знаний (`knowledge/`, в репозиторий не входит).
...
Хранилище недоступно → `503`, а не пустой `ideas` (это разные вещи для A/B-эксперимента).
