"""Idea synthesis, вертикальный кадр 800×850: гипотеза из пары идей.
...
class IdeaSynthesis(PipelineScene):
    def construct(self):
        ...
        back.set_z_index(-2)
        # Справа, а не слева: слева пунктир упирается в борт, и подпись за
        # ним обрезается на середине слова.
        self.note("derived_from", back, RIGHT, buff=0.3, color=EDGE)
        self.play(
        ...
        # Финальный кадр держим дольше прочих пауз: на нём говорят.
        self.wait(2.8)
