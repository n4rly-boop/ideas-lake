"""Feedback path, вертикальный кадр 800×850: лог прогона возвращается в озеро.

Озеро занимает верх кадра, прогон — низ: идеи уходят вниз в GigaEvo, логи
поднимаются обратно. Когда прогон отработал и коробка ушла, граф едет в
центр: дальше речь только о нём, а верхняя посадка оставляла полкадра пустым.
"""
...
class FeedbackPath(PipelineScene):
    def construct(self):
        ...
        # Коробки нет — граф едет в центр. Дальше всё считается от живых
        # центров узлов, поэтому рёбра, ореолы и кольца доверия переезжают
        # вместе с ним сами.
        graph = VGroup(edges, nodes, leaves, logs, lake)
        self.play(graph.animate.move_to(UP * 0.45), run_time=1.0)
        self.wait(0.5)
        ...
        # Под графом, в том же месте, что и прошлая подпись: новое ребро идёт
        # через всё озеро наискось, и любая подпись у него ложится на другие.
        self.note("new edge", nodes, DOWN, buff=1.0, color=DIM)
