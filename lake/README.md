...
**Ключ обязателен на каждой ручке, которая пишет в граф или тратит GPU школы:**
`Authorization: Bearer $LAKE_API_KEY`, иначе `401` (`{"error": ...}` +
`WWW-Authenticate: Bearer`). Фиксированный список чтений озера открыт БЕЗ ключа вовсе —
`GET`/`HEAD` на `/healthz`, `/stats`, `/sources(/{id})`, `/ideas(/{id}, /theses,
/neighbors)`, `/edges`, `/theses(/{id})`, `/search`, `/dial` (`OPEN_ENDPOINTS`,
`app.py`, 2026-08, граница доступа блока A) — ключ там не защищал бы ничего, читать
можно и без него. Открытость решается парой (путь, метод) по шаблону маршрута, который
FastAPI зарегистрировал, а не префиксом строки: `GET /sources` открыт, `POST /sources` —
нет, и `PATCH /ideas/{id}` не открывается тем, что `GET /ideas/{id}` открыт. Ингест
остаётся закрытым целиком, включая GET: `/ingest/jobs(/{id})`, `/ingest/staging`,
`/ingest/pending-link` — это машинное отделение, не данные озера.

Проверка — middleware в `app.py`, а не зависимость на маршруте: маршрут можно написать без
зависимости, а часть маршрутов пишет в граф и тратит GPU школы. Она стоит **до** роутинга,
поэтому несуществующий путь тоже `401`, а не `404`: чтобы узнать, какие пути есть, ключ
всё ещё нужен — кроме путей из `OPEN_ENDPOINTS`/`OPEN_PATHS`, которые отвечают данными
независимо от заголовка. Без ключа в окружении сервер **не стартует** — пустая строка
это отказ на старте, а не «аутентификация выключена»; выключается явно, флагом
`--no-auth`, и он пишет об этом в лог. Открыты также `/openapi.json`, `/docs` и `/ui` —
контракт интеграции и статическая консоль, данных озера в них нет.
